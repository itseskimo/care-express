...
import { getOngoingOrders , getCompletedOrders} from '@/redux/actions/orderActions'
...
useEffect(()=>{
  ...
  return (
    ...
    <main className='pt-8 ml-auto mr-auto w-[92%]'>
      ...
      <span className={` px-5 py-3 rounded-t-[9px]  cursor-pointer curvyBtn text-[20px] font-normal mr-2 ${navitem==='Ongoing' ? 'bg-white text-black shadow' : ''} `} onClick={(e)=>redirect(e.target.innerText) }>Ongoing</span>
      <span className={` px-5 py-3 rounded-t-[9px] cursor-pointer curvyBtn text-[20px] font-normal mr-2 ${navitem==='Completed' ? 'bg-white text-black shadow' : ''} `} onClick={(e)=>redirect(e.target.innerText) }>Completed</span>
      </nav>
      </header>

{
  navitem === 'Ongoing' ? <section className='mt-10  overflow-hidden'>
        ...
        {orderHistory?.result?.docs?.map((item)=>{
          return  <main className='bg-white grid grid-cols-[1fr,1fr,1fr,1fr,0.7fr,1.4fr] px-10 border-b-[1px] border-solid border-gray-300'>
          <h6 className='py-5 text-[14px]'>{item.created_at}</h6>
          <h6 className='py-5 text-[14px]'>{item.plan.service.charAt(0).toUpperCase() + item?.plan?.service.slice(1) } Express</h6>
          <h6 className='py-5 text-[14px]'>{item.plan.hours} Hours</h6>
          <h6 className='py-5 text-[14px]'>{item.plan.rate_per_hour} zl/h</h6>
          <h6 className='py-5 text-[14px]'>{item.plan.total_price} zl</h6>
          <h6 className='py-5 text-[14px] underline underline-offset-2 decoration-[1px] text-blue cursor-pointer flex'><img className='pr-2' src='../images/dashboard/pdf.svg'/>Download Invoice <Link href={{pathname:'/orders/viewDetails' , query: { id: item._id}}}><span className='flex pl-5'><img className='pr-2' src='../images/dashboard/eye.svg' />View Details</span></Link> </h6>
      </main>
        })}
      ...
      <main className='grid grid-cols-[1.4fr,1.4fr,1.4fr,1.4fr,1.4fr,1fr] px-10'>
        ...
        {orderHistory?.result?.docs?.map((item)=>{
         return <main className='bg-white grid grid-cols-[1.4fr,1.4fr,1.4fr,1.4fr,1.4fr,1fr] px-10 border-b-[1px] border-solid border-gray-300'>
          <h6 className='py-5 text-[14px]'>{item.created_at}</h6>
          <h6 className='py-5 text-[14px]'>{item.plan.service.charAt(0).toUpperCase() + item?.plan?.service.slice(1) } Express</h6>
          <h6 className='py-5 text-[14px]'>{item.plan.hours} Hours</h6>
          <h6 className='py-5 text-[14px]'>{item.plan.rate_per_hour} zl/h</h6>
